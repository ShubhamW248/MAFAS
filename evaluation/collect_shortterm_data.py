"""
Collect historical metrics for Short-Term Evaluation (Aug 2025 snapshot)
This script fetches historical data as it was in August 2025 for 15 diversified stocks.
Ground truth: Compare to actual performance over 2-6 months (Aug 2025 -> Oct/Dec 2025)
"""
...
# Signal thresholds for short-term (2-6 months)
def get_correct_signal_shortterm(return_pct):
    """Determine correct signal based on actual return for short-term"""
    if return_pct > 8:
        return "BUY"
    elif return_pct < -3:
        return "SELL"
...
# Define 15 diversified stocks (same as long-term)
STOCKS = [
    "AAPL",   # Tech
    "MSFT",   # Tech
    "NVDA",   # Tech
...
def get_historical_metrics(ticker, snapshot_date):
    ...
    try:
        ...
        # Calculate returns from snapshot date to now (for 2-6 month evaluation)
        current_data = stock.history(period="1d")
        if not current_data.empty:
            current_price = current_data['Close'].iloc[-1]
            period_return_2m = ((current_price - snapshot_price) / snapshot_price) * 100
        else:
        ...
        # Get 1-month and 6-month historical returns for context
        hist_1m = stock.history(start=snapshot_date - timedelta(days=30), end=snapshot_date)
        one_month_return = None
        if len(hist_1m) > 1:
            one_month_return = ((hist_1m['Close'].iloc[-1] - hist_1m['Close'].iloc[0]) / hist_1m['Close'].iloc[0]) * 100
        ...
        metrics = {
            ...
            "period_return_pct": round(period_return_2m, 2),
...
def collect_shortterm_data(output_file="shortterm_evaluation_data.csv"):
    """
    Collect Aug 2025 snapshot data for all stocks
    """
    snapshot_date = "2025-08-15"
    
    print(f"Collecting short-term evaluation data (snapshot: {snapshot_date})")
    ...
    for i, ticker in enumerate(STOCKS, 1):
        ...
        if metrics:
            ...
            row = {
                ...
                "2-6 Month Return (%)": metrics["period_return_pct"],
                ...
                "Correct Signal (Short-Term 2-6M)": get_correct_signal_shortterm(metrics["period_return_pct"]),
            }
...
if __name__ == "__main__":
    df = collect_shortterm_data("shortterm_evaluation_data.csv")
    if df is not None:
        print("\nFirst few rows:")
        print(df.head())
